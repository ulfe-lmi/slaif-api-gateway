...
_QUALIFICATION_HOOK_ENV = "SLAIF_155U_QUALIFICATION"
_QUALIFICATION_ARTIFACT_ENV = "SLAIF_155U_REJECTION_ARTIFACT"
_QUALIFICATION_ROOT_ENV = "SLAIF_155U_REJECTION_ROOT"
...
def _qualification_fields(value: Mapping[object, object]) -> list[dict[str, str]]:
    ...
    for key, item in list(value.items())[:_QUALIFICATION_MAX_FIELDS]:
        name = _qualification_name(key)
        field_type = _qualification_type_class(item)
        if name in fields and fields[name] != field_type:
            fields[name] = "other"
        else:
...
def _qualification_profile(profile: ResponsesStreamValidationProfile) -> dict[str, object]:
    ...
    return {
        "codex_reasoning_events": profile.codex_reasoning_events is True,
        "codex_0149_function_tool_events": profile.codex_0149_function_tool_events is True,
...
def _record_qualification_rejection(
    ...
    *,
    ...
    """Persist one bounded event shape only for the disposable qualification."""
    if os.environ.get(_QUALIFICATION_HOOK_ENV) != "1":
        return
    if (
        profile.codex_reasoning_events is not True
        or profile.codex_0149_function_tool_events is not True
        or profile.codex_streaming_tool_events is not True
        or not profile.declared_client_tools
        or len(profile.declared_client_tools) > 32
        or profile.web_search is not False
    ):
        return
    ...
    if (
        os.path.dirname(artifact) != root
        or os.path.basename(artifact) != "qualification-rejection.json"
    ):
        return
    try:
    ...
    for key, item in list(event.items())[:_QUALIFICATION_MAX_FIELDS]:
        if isinstance(item, Mapping):
            name = _qualification_name(key)
            nested[name] = _qualification_fields(item)
            ...
            async for chunk in adapter.stream_response(provider_request):
                ...
                if not stream_event_validator.validate(chunk.json_body):
                    ...
                    _record_qualification_rejection(
                        chunk.json_body if isinstance(chunk.json_body, Mapping) else None,
