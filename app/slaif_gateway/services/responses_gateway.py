...
_QUALIFICATION_HOOK_ENV = "SLAIF_155R_QUALIFICATION"
_QUALIFICATION_ARTIFACT_ENV = "SLAIF_155R_REJECTION_ARTIFACT"
_QUALIFICATION_ROOT_ENV = "SLAIF_155R_REJECTION_ROOT"
...
def _qualification_name(value: object, *, event: bool = False) -> str:
    if not isinstance(value, str):
        return "other"
    if (len(value) > 128 if event else len(value) > 64):
        return "other"
...
def _qualification_fields(value: Mapping[str, object]) -> list[dict[str, str]]:
    fields: dict[str, str] = {}
...
def _record_qualification_rejection(
    ...
    *,
    ...
    """Write one bounded rejection shape only in the disposable qualification env."""
    ...
    if os.path.dirname(artifact) != root:
        return
    try:
