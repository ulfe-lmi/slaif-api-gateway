...
if concurrency_limit > 0 then
  redis.call("ZREMRANGEBYSCORE", concurrency_key, "-inf", now_ms - window_ms)
  ...
  if not existing and current_concurrency >= concurrency_limit then
    return {0, "concurrency", 0, 0, current_concurrency, reset_ms, window_seconds}
  end
end
...
if request_limit > 0 and current_requests + 1 > request_limit then
  return {0, "requests", math.max(request_limit - current_requests, 0), 0, 0, reset_ms, window_seconds}
end
...
if token_limit > 0 and current_tokens + estimated_tokens > token_limit then
  return {0, "tokens", 0, math.max(token_limit - current_tokens, 0), 0, reset_ms, window_seconds}
end
...
if concurrency_limit > 0 then
  redis.call("ZADD", concurrency_key, now_ms, request_id)
  redis.call("EXPIRE", concurrency_key, window_seconds)
...
return {1, "allowed", remaining_requests, remaining_tokens, concurrency_in_use, reset_ms, 0}
"""
...
class RedisRateLimitService:
    ...
    async def cleanup_expired_concurrency(
        ...
        *,
        ...
        now_dt = now or datetime.now(UTC)
        cutoff_ms = int((now_dt - timedelta(seconds=window_seconds)).timestamp() * 1000)
        key = self._concurrency_key(gateway_key_id)
        try:
