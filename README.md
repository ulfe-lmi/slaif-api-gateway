...
- Responses hosted/provider-side tools, MCP/connectors, web/file search, code
  ...
  lifecycle, conversations, response cancel/list/input-item endpoints,
  and multimodal output. The implemented Responses subset remains text-output
  only, non-streaming for stored create, owned previous-response chaining, and
  owned retrieve/delete, and local/client-side for tools.
