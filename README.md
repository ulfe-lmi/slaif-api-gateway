...
- Admin web authentication foundation with `/admin/login`, `/admin/logout`, a placeholder `/admin` dashboard, key list/detail pages with CSRF-protected create, suspend/activate/revoke, validity-window, PostgreSQL hard quota limit, usage-counter reset, rotation, and create/rotate email-delivery mode actions, read-only owner/institution/cohort pages, read-only provider/route/pricing/FX pages, usage/audit activity pages, and email delivery pages with CSRF-protected send-now/enqueue actions for valid pending key deliveries, secure cookie settings, server-side session rows, and CSRF-protected forms.
