...
- [`docs/responses-compatibility.md`](docs/responses-compatibility.md) for the
  current stateless local Responses subset and future RC2 Responses contract.
...
- Responses hosted/provider-side tools, MCP/connectors, web/file search, code
  ...
  lifecycle, `previous_response_id`, conversations, response
  cancel/list/input-item endpoints,
  and multimodal output. The implemented Responses subset remains stateless,
  text-output only except for non-streaming stored create plus owned
  retrieve/delete, and local/client-side for tools.
