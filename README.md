...
- Admin web authentication foundation with `/admin/login`, `/admin/logout`, a placeholder `/admin` dashboard, key list/detail pages with CSRF-protected create, suspend/activate/revoke, validity-window, PostgreSQL hard quota limit, usage-counter reset, rotation, and create/rotate email-delivery mode actions, read-only owner/institution/cohort pages, read-only provider/route/pricing/FX pages, and read-only usage/audit/email delivery pages, secure cookie settings, server-side session rows, and CSRF-protected forms.
...
- Bulk key creation forms, standalone dashboard email resend/retry actions, and state-changing management pages for owners, institutions, cohorts, pricing, routing, providers, usage, audit, and email delivery.
