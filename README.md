...
- Admin web authentication foundation with `/admin/login`, `/admin/logout`, a placeholder `/admin` dashboard, key list/detail pages with CSRF-protected suspend/activate/revoke actions, read-only owner/institution/cohort pages, read-only provider/route/pricing/FX pages, and read-only usage/audit/email delivery pages, secure cookie settings, server-side session rows, and CSRF-protected forms.
...
- Admin dashboard key create/rotate/validity/limit/usage-reset/email workflows and state-changing management pages for owners, institutions, cohorts, pricing, routing, providers, usage, audit, and email delivery.
