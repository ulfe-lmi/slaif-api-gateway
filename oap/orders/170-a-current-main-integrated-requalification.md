...
Objective 168; `main` has not moved since) and publish a new dated
verification record answering exactly one question:

> Is this exact candidate sufficiently verified for the intended
> RC/deployment posture?

This objective repeats the abandoned Objective 169's purpose against the
same candidate, with a template-conforming order, after the strategic
abandonment of PR #306 (closed 2026-09-17, see strategic review comment
on that PR). The 169 outcome has two recorded findings that shape this
order:

- **P2.3 (169 record)** — a single transient availability-class 503 on the
  `redis-concurrency` phase's strict following-completion assertion in
  consecutive Run B made AP-1 unsatisfied (Run A was `RESULT=OK` 16/16).
  The failure window showed no host OOM/pressure/container crash; the
  in-project logs were removed by mandatory no-keep cleanup, so
  root-cause attribution was not conclusive. The same phase passed in 168
  Run A, 168 Run B (which absorbed one recovered 503), and 169 Run A.
- **P4.1 (169 record)** — the 169-a order deviated from the established
  order template (missing the `PR mode: \`CREATE_NEW_PR\`` literal), which
  the candidate's governance unit test
  `tests/unit/test_oap_governance.py::test_initial_round_declares_new_pr_and_one_objective_one_pr`
  correctly rejected fail-closed, leaving the PR head's `Unit, lint, and
  migration head` check red. This order is template-conforming (the
  `PR mode:` literal above), so that check must pass on this PR head.
  The governance test is to be treated as a faithful enforcer of the
  repository order template; do not attempt to alter it or the template.

The acceptance predicates are unchanged from 169 and remain strict: the
determinism property is two consecutive default-no-keep production-appliance
harness runs, both `RESULT=OK` 16/16, on the exact candidate. If a similar
single transient 503 recurs, do not chase, retry, or re-label it: record it
with machine evidence (exact request, phase, preconditions, host evidence)
and the verdict is `RESULT=NOT-QUALIFIED` per AP-1; strategy will then
weigh the accumulated evidence (two independent requalification attempts
with the same failure class) for the follow-up decision. No pre-emptive
softening of any acceptance predicate is permitted by this order.
...
Verified by the strategic model on 2026-09-17 ~15:20 Europe/Ljubljana
...
- All nine emitted main-branch checks on the candidate are
  `completed`/`success` (verified on the merge commit; run IDs recorded in
  the abandoned 169 record and re-verifiable via the GitHub API).
- Objective 169 is terminal: PR #306 was CLOSED (not merged) on
  2026-09-17 as strategically abandoned. Its verdict
  `RESULT=NOT-QUALIFIED` for this same candidate (findings P2.3/F1 and
  P4.1/F2, as summarized above) is preserved as historical evidence on
  branch `oap/169-current-main-integrated-requalification` (report
  `oap/reports/169-a-current-main-integrated-requalification.md`, dated
  record `docs/verification/2026-09-17-current-main-integrated-requalification.md`
  on that branch only — that record is NOT on `main` and must not be
  cherry-picked into this PR).
- Objectives 166, 167, 168 are terminal and verified (PR #303 merged:
  `openai==3.9.0` qualified, `OUTCOME=A`; PR #304 merged: candidate
  `9bb81cb9` verdict `RESULT=NOT-QUALIFIED`, dated record
  `docs/verification/2026-09-17-current-main-integrated-qualification.md`;
  PR #305 merged: multi-worker metrics exposure repaired, two consecutive
  clean-room harness runs `RESULT=OK` 16/16).
- Shared `oap/active` is terminal at `169-a`.
...
- PR #291 (Objective 155) is CLOSED as superseded; its branch is retained
...
- The reusable production-appliance qualification harness
  ...
  the candidate and is byte-identical to the 168/169-verified state.
...
- The execution VM has Docker 29.1.3 and Compose 2.40.3 available, the
  `slaif`/`postgres` local cluster on 127.0.0.1:5432 (note: the 5432
  cluster carries 168-era diagnostic residue — extra roles and a changed
  local `slaif` password — so use a fresh disposable cluster for the
  integration suite), and `codex --version` = `codex-cli 0.154.0` (known
  VM-only environment fact; see AP-2).

## PR contract

- Base: `main` at `1043c3f42fb46f06a8d7952273739cefb9de6cc7`.
- Branch: `oap/170-current-main-integrated-requalification`.
- Title: `obj170: fresh integrated requalification of current main for RC posture`.
- One new PR for `170-a`; later `170-b`... rounds amend the same PR.
...
- **P1 — Clean-room proof.** Fresh clone of the remote into
  `/tmp/obj170-cleanroom/`, detached checkout of the exact PR head,
...
- **P3 — Full local matrix (clean room).**
  ...
  - A fresh user-owned disposable PostgreSQL 16 instance for the
    integration suite (e.g. `initdb` under `/tmp/obj170-pg/`,
    `127.0.0.1:5433`, own role/database; do not reuse the 5432 system
    cluster or any prior diagnostic cluster; drop the database before the
    run, stop and remove the instance after); `python -m pytest
    tests/integration -q` with `TEST_DATABASE_URL` pointed at it; report
    exact counts.
...
- **P4 — CI evidence.** Query GitHub check runs for the exact PR head and
  ...
  commit per AP-7. `Unit, lint, and migration head` must be `success` on
  the PR head (this order is template-conforming; if it still fails,
  record the exact assertion and the verdict must be
...
- **P5 — Dated record.** Publish
  ...
  line, the complete P1–P4 evidence, a short "Relationship to the
  abandoned 169 attempt" note (same candidate; 169's P2.3 transient 503
  and P4.1 order-format finding as recorded on the abandoned branch), and
  honest limitation statements (mocked-upstream qualification only; not a
  real-provider run, release decision, security certification, or
  production approval). It is dated evidence for the named candidate
  commit only — it must not be phrased as a current-state claim for later
  commits, and no existing `docs/verification/2026-*` file may be edited.
...
- The integration suite may skip exactly the local-VM `pg_dump` case (the
  `TEST_DATABASE_URL` form limitation), identical to the 167/168/169
...
- `oap/orders/170-a-current-main-integrated-requalification.md` (unchanged
  strategic work order)
- `oap/active` (`170-a`)
- `oap/reports/170-a-current-main-integrated-requalification.md` (new
  immutable report)
...
- No code change of any kind: `app/`, `tests/`, `scripts/`, `.github/`,
  ...
  allowed paths. This includes `tests/unit/test_oap_governance.py` — the
  governance test and the order template are to be treated as stable.
- No defect fixes of any kind. If P2/P3/P4 exposes a candidate-side
  ...
  objective.
- No cherry-pick, import, or re-publication of any file from the abandoned
  169 branch (its dated record and report remain on that branch only).
...
- No GitHub-settings action (branch protection, rulesets, required
  checks), no re-opening or modification of PR #306 or any other PR, and
  no Dependabot interaction.
...
- **AP-2** — Exact P3 matrix counts on the candidate, with every non-pass
  ...
  `codex --version` output; the `pg_dump` skip identity vs the 167/168/169
...
- **AP-4** — The dated record exists at the allowed path, names the exact
  ...
  form `RESULT=QUALIFIED-RC-POSTURE` or `RESULT=NOT-QUALIFIED`, contains
  the "Relationship to the abandoned 169 attempt" note, and states the
  required limitation sentences. `python
...
- **AP-7** — One immutable report
  `oap/reports/170-a-current-main-integrated-requalification.md` with the
...
- Suffix rounds (`170-b`...) are for bounded correction of this objective
  only, not for scope growth or defect fixing.

## Setup authority

- Fresh clone and venv under `/tmp/obj170-cleanroom/` (disposable; remove
  after the objective).
- Fresh disposable PostgreSQL 16 under `/tmp/obj170-pg/` on
...
release/deploy/GitHub-settings/Dependabot action, no historical-record or
abandoned-169-file imports, no PR #306 modification), CI gate state on the
final head, and the environment-only labels with proofs.
