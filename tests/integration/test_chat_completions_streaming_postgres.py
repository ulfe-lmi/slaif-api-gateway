...
def test_streaming_chat_completions_postgres_success_finalizes(
    ...
    with respx.mock(assert_all_mocked=True, assert_all_called=True) as router:
        upstream_route = router.post("https://api.openai.com/v1/chat/completions").mock(
            return_value=httpx.Response(
                ...
                headers={"x-request-id": "upstream-openai-stream-integration"},
            )
        )
...
def test_streaming_chat_completions_postgres_missing_usage_releases_reservation(
    migrated_postgres_url: str,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    run_alembic_upgrade_head(migrated_postgres_url)
    ...
    app = create_app(get_settings())
    sse_without_usage = (
        'data: {"id":"chatcmpl-stream","object":"chat.completion.chunk","choices":[]}\n\n'
        "data: [DONE]\n\n"
    )
    with respx.mock(assert_all_mocked=True, assert_all_called=True) as router:
        router.post("https://api.openai.com/v1/chat/completions").mock(
            return_value=httpx.Response(
                200,
                content=sse_without_usage.encode(),
                headers={"x-request-id": "upstream-openai-stream-missing-usage"},
            )
        )
        with TestClient(app) as client:
            with client.stream(
