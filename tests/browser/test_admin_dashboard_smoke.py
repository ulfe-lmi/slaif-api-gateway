    ...
    with _run_uvicorn_server(app, port), _chromium_browser() as browser:
        ...
        try:
            ...
            pages = [
                ...
                (f"/admin/owners/{data['owner_id']}/edit", "Edit owner", 2),
                ("/admin/institutions", "Institutions", 1),
                ("/admin/institutions/new", "Create institution", 2),
                (f"/admin/institutions/{data['institution_id']}", "Browser Institution", 1),
                (f"/admin/institutions/{data['institution_id']}/edit", "Edit institution", 2),
                ("/admin/cohorts", "Cohorts", 1),
                ("/admin/cohorts/new", "Create cohort", 2),
                (f"/admin/cohorts/{data['cohort_id']}", "Browser Cohort", 1),
                (f"/admin/cohorts/{data['cohort_id']}/edit", "Edit cohort", 2),
            ...
            for path, expected, csrf_count in pages:
                ...
                if path == "/admin/keys/create":
                    ...
                    assert "Runtime permits only exact OpenAI Responses" in html
                if path == "/admin":
                ...
                if path == f"/admin/keys/{data['key_id']}":
                    ...
                    assert "Configured policy is only one part of runtime authority" in html
                ...
                if path == "/admin/routes/new":
                    assert "Runtime consumes it only for exact OpenAI Responses" in html
