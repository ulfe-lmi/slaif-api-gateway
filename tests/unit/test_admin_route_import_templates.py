...
def test_route_import_template_includes_dry_run_limits_and_csrf(monkeypatch) -> None:
    ...
    assert "Execute create-only import" in response.text
    assert "confirm_import" in response.text
...
def test_route_import_result_template_is_no_raw_content_and_safe(monkeypatch) -> None:
    async def build_preview(request, raw_rows, *, max_rows):
        return RouteImportPreview(
            total_rows=1,
            valid_count=1,
    ...
    assert "requested_model,match_type" not in response.text
    assert "&lt;script&gt;alert(1)&lt;/script&gt;" in response.text
    assert "<script>alert(1)</script>" not in response.text
    assert "provider key values" not in response.text.lower()
    assert "token_hash" not in response.text
