...
def test_image_capability_denial_rejects_before_redis_pricing_quota_or_provider(
    monkeypatch,
) -> None:
    import slaif_gateway.services.chat_completion_gateway as main_module
    ...
    _wire_auth_and_db(monkeypatch, app)
    calls: list[str] = []
    raw_url = "https://example.test/private.png?token=raw-secret"
    ...
    assert response.status_code == 400
    assert response.json()["error"] == {
        "message": "This model route does not support Chat Completions image input.",
        "type": "invalid_request_error",
        "param": "messages",
        "code": "chat_image_input_capability_not_supported",
    }
    assert calls == ["route"]
...
def test_file_capability_denial_rejects_before_redis_pricing_quota_or_provider(
    monkeypatch,
) -> None:
    import slaif_gateway.services.chat_completion_gateway as main_module
            ...
            "messages": [
                {
                    ...
                    "content": [
                        ...
                        {
                            "type": "file",
                            "file": {"filename": raw_filename, "file_data": raw_file_data},
                        },
                    ],
                }
            ],
    ...
    assert response.status_code == 400
    assert response.json()["error"] == {
        "message": "This model route does not support Chat Completions file input.",
        "type": "invalid_request_error",
        "param": "messages",
        "code": "chat_file_input_capability_not_supported",
    }
    assert calls == ["route"]
    assert "raw prompt marker" not in response.text
    assert raw_file_data not in response.text
    assert raw_filename not in response.text
    assert "sk-slaif-raw-gateway-key" not in response.text


def test_audio_capability_denial_rejects_before_redis_pricing_quota_or_provider(
    monkeypatch,
) -> None:
    import slaif_gateway.services.chat_completion_gateway as main_module
    ...
    assert response.status_code == 400
    assert response.json()["error"] == {
        "message": "This model route does not support Chat Completions audio input.",
        "type": "invalid_request_error",
        "param": "messages",
        "code": "chat_audio_input_capability_not_supported",
    }
    assert calls == ["route"]
    assert "raw prompt marker" not in response.text
    assert raw_audio_data not in response.text
    assert "sk-slaif-raw-gateway-key" not in response.text


def test_audio_output_capability_denial_rejects_before_redis_pricing_quota_or_provider(
    monkeypatch,
) -> None:
    import slaif_gateway.services.chat_completion_gateway as main_module
    ...
    async def _fake_resolve_model(self, requested_model, authenticated_key):
        _ = (self, requested_model, authenticated_key)
        calls.append("route")
        return _route_result("gpt-audio")
    ...
    assert response.status_code == 400
    assert response.json()["error"] == {
        "message": "This model route does not support Chat Completions audio output.",
        "type": "invalid_request_error",
        "param": "modalities",
        "code": "chat_audio_output_capability_not_supported",
    }
    assert calls == ["route"]
