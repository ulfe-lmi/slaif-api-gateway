...
def test_pricing_import_execute_tsv_requires_confirmation_and_audit_reason(monkeypatch) -> None:
    called = False
    ...
    async def execute(plan, **kwargs):
        ...
        called = True
        assert plan.executable is True
        assert kwargs["reason"] == "pricing TSV import"
        return PricingImportExecutionResult(
            total_rows=1,
            created_count=1,
            updated_count=0,
    ...
    without_confirm = client.post(
        "/admin/pricing/import/execute",
        data={"csrf_token": "dashboard-csrf", "import_format": "tsv", "import_text": _valid_tsv(), "reason": "pricing TSV import"},
    )
    without_reason = client.post(
        "/admin/pricing/import/execute",
