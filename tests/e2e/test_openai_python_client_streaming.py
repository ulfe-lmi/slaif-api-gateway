...
@pytest.mark.e2e
def test_openai_python_client_chat_completions_streaming_file_input_e2e(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    database_url = _test_database_url()
    run_alembic_upgrade_head(database_url)
    _configure_runtime_environment(monkeypatch, database_url)
    created = asyncio.run(_create_test_data(database_url, owner_label="OpenAI File Stream", file_inputs=True))
    ...
    filename = "stream-notes.txt"
    first_delta = {
        "id": "chatcmpl-file-stream-e2e",
        "object": "chat.completion.chunk",
        "created": 123,
        "model": TEST_MODEL,
        "choices": [{"index": 0, "delta": {"content": "file "}, "finish_reason": None}],
    }
    second_delta = {
        "id": "chatcmpl-file-stream-e2e",
        "object": "chat.completion.chunk",
        ...
        with respx.mock(assert_all_mocked=True, assert_all_called=True) as router:
            ...
            upstream_route = router.post("https://api.openai.com/v1/chat/completions").mock(
                return_value=httpx.Response(
                    200,
                    content=sse.encode(),
                    headers={"x-request-id": "upstream-openai-file-stream-e2e"},
                )
            )
    ...
    streamed_text = "".join(chunk.choices[0].delta.content or "" for chunk in chunks if chunk.choices)
    assert streamed_text == "file answer"
    assert chunks[-1].usage is not None
    assert chunks[-1].usage.total_tokens == 35
    ...
    assert state.usage_ledger.completion_tokens == 4
    assert state.usage_ledger.total_tokens == 35
...
@pytest.mark.e2e
def test_openai_python_client_chat_completions_streaming_audio_input_e2e(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    database_url = _test_database_url()
    run_alembic_upgrade_head(database_url)
    _configure_runtime_environment(monkeypatch, database_url)
    created = asyncio.run(_create_test_data(database_url, owner_label="OpenAI Audio Stream", audio_inputs=True))
    ...
    app = create_app(get_settings())
    audio_data = "cHJpdmF0ZSBzdHJlYW0gYXVkaW8="
    first_delta = {
        "id": "chatcmpl-audio-stream-e2e",
        "object": "chat.completion.chunk",
        "created": 123,
        "model": TEST_MODEL,
        "choices": [{"index": 0, "delta": {"content": "audio "}, "finish_reason": None}],
    }
    second_delta = {
        "id": "chatcmpl-audio-stream-e2e",
        "object": "chat.completion.chunk",
    ...
    usage_delta = {
        ...
        "usage": {
            ...
            "completion_tokens": 4,
            "total_tokens": 45,
            "prompt_tokens_details": {"audio_tokens": 18},
        },
    }
    sse = _sse(first_delta) + _sse(second_delta) + _sse(usage_delta) + "data: [DONE]\n\n"
        ...
        with respx.mock(assert_all_mocked=True, assert_all_called=True) as router:
            ...
            upstream_route = router.post("https://api.openai.com/v1/chat/completions").mock(
                return_value=httpx.Response(
                    200,
                    content=sse.encode(),
                    headers={"x-request-id": "upstream-openai-audio-stream-e2e"},
                )
            )
    ...
    streamed_text = "".join(chunk.choices[0].delta.content or "" for chunk in chunks if chunk.choices)
    assert streamed_text == "audio answer"
    assert chunks[-1].usage is not None
    assert chunks[-1].usage.total_tokens == 45
    ...
    assert state.reservation.status == "finalized"
    assert state.usage_ledger.streaming is True
    assert state.usage_ledger.prompt_tokens == 41
    assert state.usage_ledger.completion_tokens == 4
    assert state.usage_ledger.total_tokens == 45
