...
from tests.e2e.test_openrouter_python_client_chat import (
    COMPLETION_TEXT,
    PROMPT_TEXT,
    TEST_MODEL,
)
...
@pytest.mark.e2e
def test_openai_python_client_chat_completions_openrouter_streaming_e2e(
    ...
    created = asyncio.run(
        _create_test_data(
            ...
            owner_label="OpenRouter Streaming",
        )
    )
    ...
    sse = (
        'data: {"id":"chatcmpl-openrouter-stream","object":"chat.completion.chunk","created":123,'
        f'"model":"{TEST_MODEL}","choices":[{{"index":0,"delta":{{"content":"Hello "}},'
        '"finish_reason":null}]}\n\n'
        'data: {"id":"chatcmpl-openrouter-stream","object":"chat.completion.chunk","created":123,'
        f'"model":"{TEST_MODEL}","choices":[{{"index":0,"delta":{{"content":"from mocked OpenRouter"}},'
        '"finish_reason":"stop"}]}\n\n'
        'data: {"id":"chatcmpl-openrouter-stream","object":"chat.completion.chunk","created":123,'
        f'"model":"{TEST_MODEL}","choices":[],"usage":'
        '{"prompt_tokens":7,"completion_tokens":8,"total_tokens":15,"cost_usd":"0.0012"}}\n\n'
        "data: [DONE]\n\n"
    )
        ...
        with respx.mock(assert_all_mocked=True, assert_all_called=True) as router:
            ...
            upstream_route = router.post(
                ...
                return_value=httpx.Response(
                    200,
                    content=sse.encode(),
                    headers={"x-openrouter-request-id": "upstream-openrouter-stream-e2e"},
                )
            )
