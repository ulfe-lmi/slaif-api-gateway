...
3. Hosted tools, MCP/connectors, file search, web search, code interpreter, image generation, video, moderations, batch, vector stores, and Responses background/cancel were not RC2 targets. Later implementation outside this scope lock does not change their RC2 classification; the `Current SLAIF status` column records current `main` separately.
...
| Realtime audio | Implemented for bounded WebRTC client-secret admission foundation with explicit direct-provider-exposure gating for quota-limited keys | Unit, forwarding, pricing, error-shape, and guarded mocked official-client E2E coverage | PostgreSQL reservation/finalization implemented for client-secret issuance; provider usage authoritative when present; quota-limited keys require explicit route acceptance plus configured non-refundable admission pricing because the provider client secret is reusable and client-overridable | Canonical OpenAI JSON forwarding implemented for `POST /v1/realtime/client_secrets`; OpenRouter fails closed | Ephemeral client secrets, instructions, audio/transcript payloads, raw session config, raw SDP, and raw events are not stored/logged | `RC2_REQUIRED_IMPLEMENTED` | RC2 implements the browser/mobile WebRTC client-secret slice, but direct provider exposure is not treated as a hard actual-usage bound and larger Realtime transports/features remain deferred | — |
| Hosted/provider-side tools | One bounded post-RC2 exception implemented: OpenAI Responses canonical `web_search`; all other hosted families fail closed | Contract, gateway, PostgreSQL, SSE, and mocked official-client coverage for that exception; policy denial coverage for others | Full-balance fence, provider token cost, per-call fee, overrun/following-block, and durable hold implemented for the exception only | Exact OpenAI Responses web-search forwarding only; OpenRouter, Chat hosted tools, MCP/connectors, and every other family denied | Content-free accounting evidence only; no query/result/source/URL storage | `RC2_EXPLICITLY_DEFERRED` | It was explicitly not an RC2 target and was implemented later without changing the locked RC2 classification | — |
...
| Web search | Implemented only for bounded OpenAI Responses canonical `web_search`; Chat search, aliases, filters/location/source controls, and OpenRouter search remain fail-closed | Contract, gateway, PostgreSQL concurrency/hold/reconciliation, SSE, negative/privacy, and mocked official-client coverage | PostgreSQL full-balance fence, provider tokens plus configured per-call fee, one admitted overrun, following-block, and durable unknown-outcome hold | Exact stateless OpenAI Responses shape only | No query, URL, source, result, citation, OAuth token, provider secret, or raw provider payload storage | `RC2_EXPLICITLY_DEFERRED` | Implemented after/outside the locked RC2 target; classification remains historical scope truth | — |
...
1. `feature/audio-endpoints-foundation` — completed
   - implemented `POST /v1/audio/speech`
   - implemented `POST /v1/audio/transcriptions`
   - implemented `POST /v1/audio/translations`
   - no Realtime
...
3. `feature/realtime-audio-foundation` — completed for the bounded first slice
   ...
   - hardened quota-limited-key behavior so reusable direct-provider client secrets require explicit `realtime.client_secret_direct_provider_exposure_accepted=true` route capability plus configured admission pricing
   - documented that client-secret issuance is not a hard actual-session-usage bound because the provider secret is reusable until expiry and the client can update session behavior after minting
   - implemented admission reservation/finalization without storing instructions, audio, transcripts, raw session config, raw SDP, raw events, or ephemeral secrets
