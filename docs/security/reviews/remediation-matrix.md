...
| 4.2 | Scheduled reconciliation or alerting | Medium | Partially addressed | This PR | Reconciliation task unit tests and PostgreSQL integration tests | Celery/Celery Beat foundation can inspect backlog and optionally execute existing expired-reservation/provider-completed reconciliation flows when explicitly configured. Defaults remain dry-run/reporting and external Slack/PagerDuty/webhook alert integrations remain future work. |
| 4.2 | Dashboard pages | Medium | Partially addressed | Later admin dashboard PR series through #100 | Admin dashboard unit and PostgreSQL integration tests | The implemented dashboard now covers auth/session/CSRF, key management, provider/route/pricing/FX metadata management, read-only activity pages, and one-time-secret-backed email delivery actions. Bulk/import workflows, owner/institution/cohort mutations, usage/audit mutations, MFA, and deployment packaging remain out of scope. |
