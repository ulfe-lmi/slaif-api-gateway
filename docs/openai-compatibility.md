...
| `POST /v1/conversations` / `GET /v1/conversations/{conversation_id}` / `DELETE /v1/conversations/{conversation_id}` | Limited | Required | No generation quota reservation or normal generation usage ledger row | Not applicable | Ownership-checked provider conversation reference foundation. Create persists only safe provider conversation reference metadata after a provider conversation object with an ID is returned. Retrieve/delete proxy only owned active local references. Conversation update is not implemented |
...
- text-output `POST /v1/responses`, stateless by default with non-streaming
  stored create only when explicitly enabled;
...
- no `input_image.file_id`, `input_file.file_id`, `input_audio`, audio output,
  ...
  output;
- owned response retrieve/delete/input-item listing only for locally referenced
  stored Responses;
- conversation create/retrieve/delete only as ownership-checked provider
  reference proxy calls with no local conversation item content storage;
- no response cancel/list endpoints.
...
Stored Responses are supported only for non-streaming `POST /v1/responses`
with explicit `store=true` and `capabilities.responses.stored_responses=true`.
After a successful provider response with an ID, SLAIF stores only safe
reference metadata needed for ownership and provider routing. `GET` and
`DELETE /v1/responses/{response_id}` require explicit key endpoint permission
and an active local reference owned by the same gateway key before any upstream
proxy call occurs. `GET /v1/responses/{response_id}/input_items` follows the
same ownership check and additionally requires
...
Non-streaming `previous_response_id` requires an active local response
reference owned by the same gateway key and a compatible provider route before
SLAIF forwards it upstream. Non-streaming `conversation` requires an active
local conversation reference owned by the same gateway key and a compatible
provider route before SLAIF forwards it upstream. Conversation update/item
endpoints, cancel, response list, compact `previous_response_id`, streaming
conversation, and streaming `previous_response_id` remain unsupported.
...
- Responses cancel/list endpoints, conversation update endpoints,
  ...
  the first ownership-checked conversation reference foundation; see
