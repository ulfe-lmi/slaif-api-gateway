...
| Admin usage/audit/email delivery read-only pages | Implemented | Not provider-specific | `/admin/usage`, `/admin/audit`, and `/admin/email-deliveries` list/detail pages show safe local metadata only; prompts, completions, raw bodies, email bodies, key material, and mutation actions are not exposed | Unit and PostgreSQL integration tests |
| Dashboard management pages | Partial | None | Key creation, key create/rotate email-delivery modes, suspend/activate/revoke, validity-window, PostgreSQL hard quota limit, usage-counter reset, and rotation actions exist; standalone email delivery resend/retry actions and owner, institution, cohort, provider, route, pricing, FX, usage, audit, and email delivery mutation pages remain out of scope for current implementation | Admin dashboard page tests |
