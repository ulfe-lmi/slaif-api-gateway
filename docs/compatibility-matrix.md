...
| Admin web auth foundation | Implemented | Not provider-specific | `/admin/login`, `/admin/logout`, and placeholder `/admin` dashboard with server-side sessions and CSRF | Unit and PostgreSQL integration tests |
...
| Admin FX read-only pages | Implemented | Not provider-specific | `/admin/fx` list/detail pages show safe local metadata only; provider key values and mutation actions are not exposed | Unit and PostgreSQL integration tests |
| Admin usage/audit/email delivery pages | Implemented | Not provider-specific | `/admin/usage` and `/admin/audit` pages show safe local metadata only. `/admin/email-deliveries` pages show safe delivery metadata and provide CSRF-protected send-now/enqueue actions only for pending/failed key deliveries backed by valid unconsumed one-time secrets; prompts, completions, raw bodies, email bodies, plaintext key material, and arbitrary resend actions are not exposed | Unit and PostgreSQL integration tests |
| Dashboard management pages | Partial | None | Key creation, key create/rotate email-delivery modes, email delivery send-now/enqueue for valid one-time secrets, suspend/activate/revoke, validity-window, PostgreSQL hard quota limit, usage-counter reset, rotation, provider config metadata actions, model route metadata actions, and pricing-rule metadata actions exist; arbitrary old-key email resend actions, pricing import/upload, and owner, institution, cohort, FX, usage, and audit mutation pages remain out of scope for current implementation | Admin dashboard page tests |
