...
| Admin route/pricing/FX read-only pages | Implemented | Not provider-specific | `/admin/routes`, `/admin/pricing`, and `/admin/fx` list/detail pages show safe local metadata only; provider key values and mutation actions are not exposed | Unit and PostgreSQL integration tests |
...
| Dashboard management pages | Partial | None | Key creation, key create/rotate email-delivery modes, email delivery send-now/enqueue for valid one-time secrets, suspend/activate/revoke, validity-window, PostgreSQL hard quota limit, usage-counter reset, rotation, and provider config metadata actions exist; arbitrary old-key email resend actions and owner, institution, cohort, route, pricing, FX, usage, and audit mutation pages remain out of scope for current implementation | Admin dashboard page tests |
