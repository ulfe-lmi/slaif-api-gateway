...
| Admin provider/route/pricing/FX read-only pages | Implemented | Not provider-specific | `/admin/providers`, `/admin/routes`, `/admin/pricing`, and `/admin/fx` list/detail pages show safe local metadata only; provider key values and mutation actions are not exposed | Unit and PostgreSQL integration tests |
