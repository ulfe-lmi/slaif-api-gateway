...
| Admin key read-only pages | Implemented | Not provider-specific | `/admin/keys` list and `/admin/keys/{gateway_key_id}` detail pages show safe metadata only; no key mutation actions | Unit and PostgreSQL integration tests |
...
| Dashboard management pages | Partial | None | Read-only key, owner, institution, cohort, provider, route, pricing, FX, usage, audit, and email delivery pages exist; state-changing web management workflows remain out of scope for current implementation | Admin dashboard page tests |
