...
| Admin key pages | Implemented | Not provider-specific | `/admin/keys` list and `/admin/keys/{gateway_key_id}` detail pages show safe metadata only; key detail supports CSRF-protected suspend, activate, revoke, validity-window, PostgreSQL hard quota limit, usage-counter reset, and rotation POST actions through the existing key service and audit behavior. Rotation shows the replacement plaintext key exactly once on a no-cache result page | Unit and PostgreSQL integration tests |
...
| Dashboard management pages | Partial | None | Key suspend/activate/revoke, validity-window, PostgreSQL hard quota limit, usage-counter reset, and rotation actions exist; key create/email actions and owner, institution, cohort, provider, route, pricing, FX, usage, audit, and email delivery mutation pages remain out of scope for current implementation | Admin dashboard page tests |
