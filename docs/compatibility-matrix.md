...
| Admin key pages | Implemented | Not provider-specific | `/admin/keys` list and `/admin/keys/{gateway_key_id}` detail pages show safe metadata only; key detail supports CSRF-protected suspend, activate, and revoke POST actions through the existing key service and audit behavior | Unit and PostgreSQL integration tests |
...
| Dashboard management pages | Partial | None | Key suspend/activate/revoke actions exist; key create/rotate/validity/limit/usage-reset/email actions and owner, institution, cohort, provider, route, pricing, FX, usage, audit, and email delivery mutation pages remain out of scope for current implementation | Admin dashboard page tests |
