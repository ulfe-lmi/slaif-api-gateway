...
- Standalone `POST /v1/embeddings` behind separate endpoint permission,
  explicit `embeddings` route capability, optional `dimensions` only when the
...
- `POST /v1/realtime/client_secrets` behind separate endpoint permission,
  ...
  config, audio, transcripts, raw SDP, or raw events. Because OpenAI client
  secrets are reusable until expiry and later session behavior is
