...
- top-level `audio`;
- `modalities` values other than text-only.
...
and provider-reported OpenRouter cost fields. It does not have modality-specific
pricing rows or finalization semantics for image, audio, or file surfaces.
...
3. File ID or file URL support, only if ownership, lifecycle, privacy, and
   pricing policy are clear.
4. Audio input to text output, only after audio-token usage/pricing and size
   caps are clear.
5. Text input to audio output, as a separate media-response PR because response
   shape, output bytes, stream deltas, and pricing are different.
6. Audio input plus audio output, only after input and output audio are
   independently supported.
7. Mixed-modality combinations and interactions with `n > 1`, local function
...
Do not start with audio output. Audio output introduces base64 media in provider
responses, ambiguous streaming deltas in the installed official SDK, audio-token
...
Uploaded file IDs and file URLs remain disabled because ownership, lifecycle,
privacy, and provider-side file fetching policy are still separate concerns.
Do not combine audio or file-ID/file-URL work in the next implementation PR.

## Decision

Keep Chat Completions file IDs, file URLs, audio input, audio output, image
generation, and broader mixed-modality support disabled on current `main`.
Image input and inline file input to text output are enabled only through
explicit route capabilities, conservative caps, provider usage/cost
finalization, provider adapter tests, official-client E2E coverage, and
redaction/no-storage tests.
