...
references, top-level audio-output controls, and unknown audio fields. SLAIF
...
4. File ID or file URL support, only if ownership, lifecycle, privacy, and
...
Audio input to text output is now implemented as the third narrow slice. Uploaded
file IDs and file URLs remain disabled because ownership, lifecycle, privacy,
and provider-side file fetching policy are still separate concerns. Do not
combine audio output or file-ID/file-URL work in the next implementation PR.

## Decision

Keep Chat Completions file IDs, file URLs, audio output, image generation, and
broader media-output support disabled on current `main`. Image input, inline
file input, and audio input to text output are enabled only through explicit
route capabilities, conservative caps, provider usage/cost finalization,
