...
2. File input to text output, only if Chat Completions file data, file IDs,
   pricing, and ownership policy are clear.
3. Audio input to text output, only after audio-token usage/pricing and size
   caps are clear.
4. Text input to audio output, as a separate media-response PR because response
   shape, output bytes, stream deltas, and pricing are different.
5. Audio input plus audio output, only after input and output audio are
   independently supported.
6. Mixed-modality combinations and interactions with `n > 1`, local function
