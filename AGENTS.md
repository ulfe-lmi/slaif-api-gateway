...
- `gateway_keys.hmac_key_version` records which HMAC secret version was used.
...
Key statuses stored in PostgreSQL:
...
Routing must be data-driven via `model_routes`, not hardcoded only in Python.
...
Use an explicit local pricing/model catalog backed by `pricing_rules`.
...
Each request should record the fields defined in `docs/database-schema.md`, including at least:

- request ID generated by gateway
- upstream provider request ID if available
- gateway key ID
- owner/institution/cohort/workshop metadata snapshot
- endpoint
- provider
- requested model
...
- cached tokens
- reasoning tokens
- other token categories if applicable
- estimated reserved cost/tokens
- final cost in EUR and native currency where applicable
- cost source: gateway_calculated, provider_reported, manual, estimated, unknown
- response status
...
Do not store full prompts/responses by default. For privacy, content logging must be opt-in and explicitly documented.
...
The authoritative database schema for this project is documented in:

```text
docs/database-schema.md
```

Codex must implement SQLAlchemy models and Alembic migrations from that file.

Do not duplicate a competing inline schema in this AGENTS.md. Do not invent additional schema fields, remove tables, or simplify accounting logic without updating `docs/database-schema.md` and documenting the reason.

The schema uses PostgreSQL 16+ with SQLAlchemy 2.x async, asyncpg, and Alembic.

Important schema requirements:

- PostgreSQL is the production source of truth.
- Redis is only for fast temporary state such as rate-limit counters, distributed locks, quota coordination, and Celery broker data.
- Gateway-issued API keys must never be stored in plaintext.
- Gateway keys must be stored as HMAC-SHA-256 hashes using a server-side pepper/secret supplied outside the database.
- HMAC key versioning must be supported as described in this file and `docs/database-schema.md`.
- Provider API keys such as OpenAI and OpenRouter keys must not be stored in plaintext in PostgreSQL.
- Provider API keys must be supplied through environment variables or Docker secrets.
- User-facing cost limits are expressed in EUR.
- If upstream pricing or returned cost is in another currency, the service must convert to EUR using configured FX data before quota reservation and finalization.
- Quota enforcement must use a reserve-then-finalize model.
- Concurrent requests must not be able to overspend the same gateway key.
- The `quota_reservations` table is required and must not be removed.
- The `usage_ledger` table is the accounting record for proxied requests.
- Prompt, completion, uploaded file, and tool payload contents must not be stored by default.
- The `one_time_secrets` table is required for asynchronous key email delivery.
- `one_time_secrets` must store encrypted temporary secret material, not plaintext.
- Celery jobs must reference `one_time_secrets.id` and must not carry plaintext API keys in Redis job payloads.
- The `audit_log` table is required for security-relevant admin and CLI actions.
- The `provider_configs` table must store provider metadata and environment variable names only, never provider secret values.
- The `model_routes` and `pricing_rules` tables are required for explicit routing and fail-closed pricing.
- Unknown models or unknown pricing must be rejected unless explicitly enabled by an admin and documented.
...
Minimum required tables from `docs/database-schema.md`:
...
- Provide a one-shot migration command/container, for example:
...
4. Gateway key generation and HMAC validation with key versioning.
5. Admin user creation CLI.
6. Basic `/v1/chat/completions` non-streaming proxy with mocked tests.
7. Provider routing table and adapters for OpenAI/OpenRouter.
8. Quota reservation and usage ledger.
9. Gateway output-token/default cap policy.
10. Streaming proxy with SSE tests.
11. Pricing table, FX conversion, and cost accounting.
12. Admin dashboard login and key list/create/revoke/suspend/activate.
13. One-time secret encryption and email delivery via Celery/Mailpit.
14. Usage reports and CSV export.
15. Redis rate limiting.
16. Prometheus metrics and structured logging.
17. Nginx deployment example.
18. Full docs and compatibility matrix.
19. Optional upstream smoke tests.
20. Optional Playwright dashboard tests.
21. Optional OpenTelemetry tracing.
